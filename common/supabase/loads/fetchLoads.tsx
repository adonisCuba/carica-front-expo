...
export const fetchLoads = async ({
  isDriver = false,
}: FetchLoadsProps): Promise<Load[] | undefined> => {
  let fullLoads: Load[] = [];
...
export const fetchLoadsByLocation = async (location: string) => {
  ...
  let { data: ubicacionInicial, error: errorUbicacion } = await supabase
    .from("ubicaciones")
    .select("*")
...
export const loadsMapper = async (
  carga: LoadSupabaseUnit
  ...
  try {
    ...
    const { data: formaDePago, error: errorFormaDePago } = await supabase
      ...
      .maybeSingle();
    if (errorFormaDePago || !formaDePago)
      throw new Error("Error obteniendo forma de pago");
    ...
    const { data: presentacion, error: errorPresentacion } = await supabase
      ...
      .maybeSingle();
    if (errorPresentacion || !presentacion)
      throw new Error("Error obteniendo presentación");
    ...
    if (errorMaterial || !material)
      throw new Error("Error obteniendo material");

    const { data: ubicacionInicial, error: errorUbicacionInicial } =
      await supabase
        .from("ubicaciones")
        .select("direccion")
        .eq("id", carga.ubicacioninicial_id)
        .maybeSingle();
    if (errorUbicacionInicial || !ubicacionInicial)
      throw new Error("Error obteniendo ubicación inicial");
    ...
    const { data: ubicacionFinal, error: errorUbicacionFinal } = await supabase
      ...
      .maybeSingle();
    if (errorUbicacionFinal || !ubicacionFinal)
      throw new Error("Error obteniendo ubicación final");
    const { data: tipoEquipo, error: errorTipoEquipo } = await supabase
      .from("tipoequipo")
      .select("descripcion")
      .eq("id", carga.tipo_equipo)
      .maybeSingle();
