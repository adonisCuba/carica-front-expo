...
export const editLoad = async (loadId: string, load: any) => {
  ...
  try {
    ...
    const { data: latLngCarga } =
      await ServiceLocation.getGoogleGeocodingLatLng(newDirectionLoad);
    coordsCarga = latLngCarga;
    if (!coordsCarga)
      throw new Error("No se pudo actualizar la ubicación de carga");

    // Obtiene coordenadas para descarga
    const { data: latLngDescarga } =
      await ServiceLocation.getGoogleGeocodingLatLng(newDirectionDownload);
    coordsDescarga = latLngDescarga;
    if (!coordsDescarga)
      throw new Error("No se pudo actualizar la ubicación de descarga");
  ...
  try {
    ...
    const updateLoadData: Partial<
      Database["public"]["Tables"]["cargas"]["Update"]
    > = {
      peso: peso.toString(),
