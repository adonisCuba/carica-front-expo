...
import { editLoad } from "@/common/supabase/loads/editLoad";
...
const CreateLoad = () => {
  ...
  const [selectDownloadDate, setSelectDownloadDate] = useState(
    fechaDescarga !== null
  );
  ...
  const handleLoad = async (values: any) => {
    setLoading(true);
    if (id) await editLoad(id as string, values);
    else await createLoad(values);
    navigation.goBack();
  ...
  return (
    <View style={{ flex: 1, backgroundColor: APPCOLORS.tertiary }}>
      ...
      <View>
        <ScreenHeader add={id ? false : true} />
        <KeyboardAvoidingView
            ...
            <View style={styles.container}>
              ...
              <Formik
                initialValues={{
                  ...
                  tipoCarga: presentacion as string,
                  peso: peso as string,
                  tipoEquipo: tipoEquipo as string,
                  correo: email as string,
                  telefono: telefonoDador as string,
                  puntoReferencia:
                    puntoReferencia == " " || puntoReferencia == ""
                      ? ""
                      : (puntoReferencia as string),
                  precio: valorViaje as string,
                  localidadCarga: ubicacionInicial as string,
                  localidadDescarga: ubicacionFinal as string,
                  fechaDescarga:
                    fechaDescarga == null
                      ? new Date()
                      : moment(fechaDescarga, "DD/MM/YYYY").toDate(),
                  fechaCarga:
                    fechaCarga == null
                      ? new Date()
                      : moment(fechaCarga, "DD/MM/YYYY").toDate(),
                }}
                  ...
                  >
                    <View style={styles.inputContainer}>
                      <View style={styles.inputContent}>
                        ...
                        <CustomPicker
                          ...
                          placeholder="Material a transportar"
                        />
                      </View>
                    </View>
                    <View
                    ...
                    >
                      <View style={{ flex: 1 }}>
                        <View style={[styles.inputContent, { height: 55 }]}>
                          ...
                          <CustomPicker
                            selectedValue={values.tipoCarga}
                            onValueChange={handleChange("tipoCarga")}
                            data={LoadUnitArray}
                    ...
                    <View style={styles.inputContainer}>
                      <View style={styles.inputContent}>
                        ...
                        <CustomPicker
                          selectedValue={values.tipoEquipo}
                          onValueChange={handleChange("tipoEquipo")}
                          data={LoadTypeArray}
                      ...
                      {Platform.OS !== "ios" ? (
                        ...
                        <>
                          ...
                          <RNDateTimePicker
                            mode="date"
                            value={values.fechaCarga}
                            onChange={() => handleChange("fechaCarga")}
                          />
                        </>
                      )}
                    </View>
                    ...
                    >
                      {!selectDownloadDate ? (
                        <TouchableOpacity
                          ...
                          onPress={() => setSelectDownloadDate(true)}
                        >
                          <Text
                            style={{
                          ...
                          >
                            Seleccionar Fecha de Descarga (opcional)
                          </Text>
                        </TouchableOpacity>
                      ) : null}
                      {selectDownloadDate &&
                        (Platform.OS !== "ios" ? (
                          ...
                          <>
                            <TouchableOpacity
                              ...
                              onPress={() => setSelectDownloadDate(false)}
                            >
                              <Ionicons
                                name="close-outline"
                                size={20}
...
const ScreenHeader = ({ add = true }: { add?: boolean }) => {
  return (
    <View style={styles.screenTitleContainer}>
      <View>
        <Text style={styles.screenTitle}>
          {add ? "Crear" : "Editar"} Cargas
        </Text>
        <Text style={styles.screenSubtitle}>
          Recuerda ingresar todos los datos:
        </Text>
      </View>
    </View>
  );
